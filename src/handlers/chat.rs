...
pub async fn chat_completions(
    ...
    // Route OpenAI models to OpenAI handler
    ...
    let span = tracing::span!(tracing::Level::INFO, "chat_completions", request_id = %request_id, model = %req.model, stream = req.stream);
    ...
    if req.stream {
        // Streaming Path
        let stream_result = provider.execute_stream(req.clone(), &state).await;

        let stream = match stream_result {
            Ok(provider_stream) => {
                provider_stream.map(move |chunk_result| match chunk_result {
                    Ok(chunk_data) => {
                        // chunk_data is SSE-formatted string from provider (e.g., "data: {...}\n\n")
                        // Parse the SSE line
                        if let Some(json_data) = chunk_data.strip_prefix("data: ") {
                            ...
                            match serde_json::from_str::<ChatCompletionChunk>(json_data) {
                                ...
                                Err(_) => {
                                    // Fallback: try as generic JSON value
                                    match serde_json::from_str::<Value>(json_data) {
                                        Ok(value) => match Event::default().json_data(value) {
                                            ...
                                            Err(e) => {
                                                ...
                                                Ok::<Event, Infallible>(
                                                    Event::default()
                                                        .comment("error: serialization failed"),
                                                )
                                            }
                                        },
                                        Err(_) => {
                                            // Not JSON, treat as comment
                                            Ok::<Event, Infallible>(
                                                Event::default().comment(json_data),
                                            )
                                        }
                                    }
                                }
                            ...
                            // Not SSE format, treat as comment
                            Ok::<Event, Infallible>(Event::default().comment(chunk_data.trim()))
                        }
                    }
                    Err(e) => {
                        error!("Provider stream error: {}", e);
                        Ok::<Event, Infallible>(Event::default().comment(format!("error: {}", e)))
                    }
                })
            }
            Err(e) => {

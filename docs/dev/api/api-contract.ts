...
export interface ChatCompletionRequest {
    ...
    top_p?: number;
    stream?: boolean;
    stop?: string[]; // Code expects array: Option<Vec<String>>
    max_tokens?: number;
    // n?: number; // Ignored/Unsupported
    // presence_penalty?: number; // Ignored/Unsupported
    // frequency_penalty?: number; // Ignored/Unsupported
    // logit_bias?: Record<string, number>; // Ignored/Unsupported
    // user?: string; // Ignored/Unsupported

    // Extended options for specific providers (passed through if supported)
    // response_format?: { type: 'text' | 'json_object' }; // Ignored/Unsupported
}
